$(document).ready(function() {
  ...
  if(localStorage.getItem("switched") &&  localStorage.getItem("switched")=='true'){
    $('.switcher img').addClass("rolate");
  } else{
    $('.switcher img').removeClass("rolate");
  }
  $('.switcher').click(function() {
    $('.switcher img').toggleClass('rolate');
    localStorage.setItem("switched", $('.switcher img').hasClass("rolate"));
  });
  $(".test").click(function() {
  ...
  chrome.storage.local.get("lang_selected", function(e) {
    ...
    $("#translate-text").click(function() {
      if(reg.test($('#text-input')[0].value)){
  ...
  function get_result(str, from, to) {
    if (str.length <= 150) {
      $('.en').html('<i class="speak" data-src="' + from + '" speak-for="' + str + '"></i> ' + str);
    } else {
      $('.en').html(str);
    }
    $('.vi').html(`<img src="` + image + `">`);
    if ($('#text-input')[0].value == "") {
      $('.result').slideUp(50);
    } else {
      $('.result').slideDown(50);
    }
  ...
  function append_addmore(to, target) {
    ...
    $('p').click(function() {
      window.open($('p').attr('href'), '_blank');
    })
  }
})